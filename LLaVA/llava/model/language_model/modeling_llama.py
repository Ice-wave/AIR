...
def air_qk_spectral_rescale(attn_weights: torch.Tensor, scale: float = 1.0, eps: float = 1e-8) -> torch.Tensor:
    """AIR — line 1: adaptively rescale the query-key mapping by spectral energy.

    Rescale each head's pre-softmax logit matrix S^{(l,h)} so its Frobenius energy
    ``||S||_F^2`` matches ``scale`` times the cross-head mean. Applied before softmax;
    changes per-head magnitude only, not relative structure.

    Args:
        attn_weights: (bsz, num_heads, q, k) pre-softmax attention logits.
        scale: target energy relative to cross-head mean; 1.0 = match mean.
        eps: numerical stability term.
    """
    # Compute energy in float32 to avoid fp16 overflow on large prefill matrices.
    a32 = attn_weights.to(torch.float32)
    energy = a32.pow(2).sum(dim=(-2, -1), keepdim=True)  # (bsz, H, 1, 1)
    target = scale * energy.mean(dim=1, keepdim=True)    # cross-head mean energy
    factor = torch.sqrt(target / (energy + eps))
    # Clamp to avoid extreme rescaling that collapses softmax (empty generation).
    factor = factor.clamp(0.8, 1.25)
    return (a32 * factor).to(attn_weights.dtype)


def air_variance_constrained_projection(
    attn_weights: torch.Tensor, beta: float = 0.1, eps: float = 1e-8
) -> torch.Tensor:
    """AIR — Variance-Constrained Projection Regularization (paper §5.2, Lines 12–14).

    On post-softmax attention A^{(l,h)}:
      1. Zero-trace projection (square prefill only):
         Â = A - (tr(A)/L) · I
      2. Frobenius energy rescale (prevent collapse/explosion):
         Ã = Â · sqrt(||A||_F^2 / (||Â||_F^2 + eps))
      3. Shrinkage toward matrix mean (reduce excess dispersion):
         A* = (1 - beta) · Ã + beta · mean(Ã) · 1

    Decode steps (q != k): skip steps 1–2 (trace undefined); apply shrinkage only.
    Computed in float32 for stability; returned in input dtype.

    Args:
        attn_weights: (bsz, num_heads, q, k) post-softmax attention weights.
        beta: shrinkage β∈[0,1]; larger → closer to mean matrix.
        eps: numerical stability term.
    """
    orig_dtype = attn_weights.dtype
    a = attn_weights.to(torch.float32)
    q_len, k_len = a.shape[-2], a.shape[-1]

    if q_len == k_len:
        L = k_len
        diag = torch.diagonal(a, dim1=-2, dim2=-1)        # (bsz, H, L)
        tr = diag.sum(dim=-1)                              # (bsz, H)
        a_hat = a.clone()
        idx = torch.arange(L, device=a.device)
        a_hat[..., idx, idx] = a_hat[..., idx, idx] - (tr / L).unsqueeze(-1)

        fro_a = a.pow(2).sum(dim=(-2, -1), keepdim=True)
        fro_hat = a_hat.pow(2).sum(dim=(-2, -1), keepdim=True)
        a_tilde = a_hat * torch.sqrt(fro_a / (fro_hat + eps))
    else:
        a_tilde = a

    mean = a_tilde.mean(dim=(-2, -1), keepdim=True)
    a_star = (1.0 - beta) * a_tilde + beta * mean
    return a_star.to(orig_dtype)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # AIR — line 1: spectral-energy rescale (default off; needs --air-qk-rescale).
        if (
            getattr(self.config, "air_intervention", False)
            and getattr(self.config, "air_qk_rescale", False)
            and self.layer_idx is not None
        ):
            alo = int(getattr(self.config, "air_layer_low", 5))
            ahi = int(getattr(self.config, "air_layer_high", 18))
            if alo <= self.layer_idx < ahi:
                attn_weights = air_qk_spectral_rescale(
                    attn_weights,
                    scale=float(getattr(self.config, "air_qk_scale", 1.0)),
                    eps=float(getattr(self.config, "air_eps", 1e-8)),
                )

        # AIR — modality rebalancing (vision/system logit scaling), pre-mask.
        # Optional dynamic vision boost at decode time (q_len<=img_end): schedule g by
        # n_txt=kv_seq_len-img_end to offset visual share dilution as post-image text grows; d stays constant.
        if getattr(self.config, "air_intervention", False) and self.layer_idx is not None:
            mlo = int(getattr(self.config, "air_mod_layer_low", 9))
            mhi = int(getattr(self.config, "air_mod_layer_high", 15))
            if mlo <= self.layer_idx < mhi:
                sys_len = int(self.config.img_start_pos)
                img_end = sys_len + int(self.config.img_length)
                g_base = float(getattr(self.config, "air_gamma_img", 1.08))
                d = float(getattr(self.config, "air_delta_sys", 0.97))

                schedule = str(getattr(self.config, "air_gamma_schedule", "const"))
                g_max = float(getattr(self.config, "air_gamma_img_max", g_base))
                use_dyn = (
                    q_len <= img_end
                    and schedule in ("exp", "log")
                    and g_max > g_base
                )
                if use_dyn:
                    n_txt = max(0, int(kv_seq_len) - img_end)
                    tau = max(1.0, float(getattr(self.config, "air_gamma_tau", 32.0)))
                    if schedule == "exp":
                        g = g_base + (g_max - g_base) * (1.0 - math.exp(-n_txt / tau))
                    else:  # "log"
                        kappa = float(getattr(self.config, "air_gamma_kappa", 0.05))
                        g = min(g_max, g_base + kappa * math.log(1.0 + n_txt / tau))
                else:
                    g = g_base

                if q_len > img_end:
                    attn_weights[:, :, img_end:, sys_len:img_end] *= g
                    attn_weights[:, :, img_end:, :sys_len] *= d
                else:
                    attn_weights[:, :, :, sys_len:img_end] *= g
                    attn_weights[:, :, :, :sys_len] *= d
        ...
        # AIR — module: cross-head lens on vision keys, post-mask pre-softmax
        if getattr(self.config, "air_intervention", False) and self.layer_idx is not None:
            llo = int(getattr(self.config, "air_lens_layer_low", 5))
            lhi = int(getattr(self.config, "air_lens_layer_high", 18))
            if llo <= self.layer_idx < lhi:
                img_s = int(self.config.img_start_pos)
                img_e = min(img_s + int(self.config.img_length), attn_weights.size(-1))
                if img_e > img_s:
                    alpha = float(getattr(self.config, "air_alpha_lens", 0.28))
                    vis = attn_weights[:, :, -1:, img_s:img_e]
                    attn_weights[:, :, -1:, img_s:img_e] = vis + alpha * vis.abs().mean(dim=1, keepdim=True)

        # upcast attention to fp32
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)

        # AIR — module: conditional AD-HH (post-softmax)
        if getattr(self.config, "air_intervention", False):
            if getattr(self.config, "air_conditional_adhh", True) and head_list is not None:
                thresh = float(getattr(self.config, "air_adhh_threshold", 0.5))
                for head in head_list:
                    aggre_attention = torch.sum(
                        attn_weights[:, head, -1, self.config.img_start_pos + self.config.img_length :]
                    )
                    if aggre_attention >= thresh:
                        attn_weights[:, head, -1, self.config.img_start_pos + self.config.img_length :] = 0

        # AIR — variance-constrained projection (zero-trace + Frobenius rescale + shrinkage, paper §5.2).
        if getattr(self.config, "air_intervention", False) and self.layer_idx is not None:
            alo = int(getattr(self.config, "air_layer_low", 5))
            ahi = int(getattr(self.config, "air_layer_high", 18))
            if alo <= self.layer_idx < ahi:
                attn_weights = air_variance_constrained_projection(
                    attn_weights,
                    beta=float(getattr(self.config, "air_beta", 0.1)),
                    eps=float(getattr(self.config, "air_eps", 1e-8)),
                )
